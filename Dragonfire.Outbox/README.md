...
[![CI](https://github.com/outboxnet/Dragonfire.Outbox/actions/workflows/ci.yml/badge.svg)](https://github.com/outboxnet/Dragonfire.Outbox/actions/workflows/ci.yml)
[![NuGet](https://img.shields.io/nuget/v/Dragonfire.Outbox.Core.svg)](https://www.nuget.org/packages/Dragonfire.Outbox.Core)
[![License: MIT](https://img.shields.io/badge/License-MIT-yellow.svg)](LICENSE)

A transactional outbox library for .NET that guarantees reliable webhook delivery in distributed systems. Dragonfire.Outbox ensures that when your application writes data and needs to notify external systems, either **both happen or neither does** — eliminating the class of bugs where your database commits but the notification is silently lost.
...
| Package | Description |
|---|---|
| `Dragonfire.Outbox.Core` | Core contracts, models, options, observability |
...
**Direct SQL — generate the schema from a temporary EF Core migration** or write it manually. The three tables are `OutboxMessages`, `WebhookSubscriptions`, and `DeliveryAttempts`. See `Dragonfire.Outbox.EntityFrameworkCore/Configurations/` for exact column definitions.
...
Use `X-Outbox-Message-Id` + `X-Outbox-Subscription-Id` as a composite idempotency key. Dragonfire.Outbox provides at-least-once delivery, so your receiver should be idempotent:
...
Dragonfire.Outbox runs two concurrent processing loops inside every host instance. They have different responsibilities and are designed to complement each other, not compete.
...
In a real deployment you typically run multiple API replicas (for scale) and one or more Azure Functions (for serverless processing). All share the same SQL Server database. Dragonfire.Outbox is designed for this topology from the ground up.
...
Dragonfire.Outbox emits OpenTelemetry signals out of the box.
...
| If you... | Use |
|---|---|
| Already use EF Core and want migrations + DbContext integration | `Dragonfire.Outbox.EntityFrameworkCore` |
| Use Dapper, raw ADO.NET, or want zero EF Core overhead | `Dragonfire.Outbox.SqlServer` |
| Need outbox writes in the same transaction as your EF Core `SaveChangesAsync` | `Dragonfire.Outbox.EntityFrameworkCore` |
| Need outbox writes in the same transaction as a raw `SqlTransaction` | `Dragonfire.Outbox.SqlServer` |
...
Dragonfire.Outbox/
├── src/
│   ├── Dragonfire.Outbox.Core/                    # Contracts, models, options, observability
