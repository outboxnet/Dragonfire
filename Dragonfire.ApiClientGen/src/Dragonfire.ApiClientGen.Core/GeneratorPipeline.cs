...
using Dragonfire.ApiClientGen.Emit;
using Dragonfire.ApiClientGen.Postman;
using Dragonfire.ApiClientGen.Schema;

namespace Dragonfire.ApiClientGen;
...
public sealed class GeneratorPipeline
{
    ...
    {
        var collectionJson = File.ReadAllText(options.InputPath);
        var collection = JsonSerializer.Deserialize<PostmanCollection>(collectionJson, JsonOpts())
            ?? throw new InvalidOperationException($"Failed to parse Postman collection at '{options.InputPath}'.");

        var responseExamples = LoadResponseExamples(options.ResponseExamplesPath);

        var parser = new PostmanParser(new ParseOptions
        {
        ...
        var ir = parser.Parse(collection);
