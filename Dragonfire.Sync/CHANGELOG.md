...
- `samples/Dragonfire.Sync.Sample.AzureFunctions` demonstrating a `[TimerTrigger]`
...
- `ISyncStateStore` / `ISyncStateReader` for tracking per-provider sync state
  ...
  entities.
- `Dragonfire.Sync.EntityFrameworkCore` package with `EfSyncStateStore` and a generic
  EF-Core repository.
- Built-in metrics on the `Dragonfire.Sync` `Meter` and tracing on the `Dragonfire.Sync`
  `ActivitySource`.
- Sample app (`samples/Dragonfire.Sync.Sample.WebApi`) demonstrating a worker that
