...
| Package | Purpose |
| --- | --- |
| `Dragonfire.Sync.Abstractions` | Interfaces only — depend on this from your domain projects. |
| `Dragonfire.Sync.Core` | Orchestrator, retry, circuit breaker, metrics, tracing. |
| `Dragonfire.Sync.EntityFrameworkCore` | Optional EF Core implementations of `ISyncStateStore` and a generic repository. |
...
Dragonfire.Sync has two entry points and you pick whichever matches your host:
...
Full sample under `samples/Dragonfire.Sync.Sample.AzureFunctions`.
...
* **Metrics** — `synclib.sync.duration`, `synclib.sync.records`,
  ...
  `Dragonfire.Sync` Meter.
* **Tracing** — `Dragonfire.Sync` `ActivitySource` emits a span per run with
  `sync.provider`, `sync.records`, and `sync.status` tags.

Sample app under `samples/Dragonfire.Sync.Sample.WebApi` exposes a `/sync/state`
