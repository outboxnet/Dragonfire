...
3. [How Dragonfire.Saga works internally](#3-how-saganet-works-internally)
...
Handling all of this manually leads to brittle, tangled code. Dragonfire.Saga gives you a clean, structured way to describe the process and handles the hard parts — retrying, persisting state, and rolling back — automatically.
...
Dragonfire.Saga persists each instance in a SQL database, so even if your application restarts, the workflow picks up exactly where it left off.

---

### Step

A **step** is a single item on the checklist. You write a C# class for each step. The class has one job: do its work, then tell Dragonfire.Saga what to do next.
...
In Dragonfire.Saga, you mark a step as *compensatable* by implementing `ICompensatableStep<T>`. If any later step fails, Dragonfire.Saga automatically calls the `CompensateAsync` method of each previously-completed step, in reverse order.
...
You never write the "walk back" logic yourself — Dragonfire.Saga drives it.
...
Execution pointers are persisted alongside the workflow instance in the database. This is what allows Dragonfire.Saga to resume correctly after a crash.

---

## 3. How Dragonfire.Saga works internally
...
Dragonfire.Saga looks at which steps ran before the failure:
...
```xml
<PackageReference Include="Dragonfire.Saga.Core" Version="1.0.0" />
<PackageReference Include="Dragonfire.Saga.Persistence.EfCore" Version="1.0.0" />
<PackageReference Include="Dragonfire.Saga.Extensions.DependencyInjection" Version="1.0.0" />
```
...
services.AddDbContext<SagaNetDbContext>(opt =>
    opt.UseSqlServer(Configuration.GetConnectionString("Dragonfire.Saga")));
...
```bash
dotnet ef migrations add InitialCreate --project Dragonfire.Saga.Persistence.EfCore
...
If failure here does not imply failure everywhere, it should be separate so that Dragonfire.Saga can retry just that step without re-running what already succeeded.
...
Dragonfire.Saga commits between steps. If two operations must be atomic (either both happen or neither does), they must live in the same step and share a single `DbContext` transaction.
...
Dragonfire.Saga wakes up the waiting workflow and resumes from the next step.

---

## 10. Configuring retry behaviour

By default, Dragonfire.Saga retries any step that throws an unhandled exception. You can configure this globally or per step.
...
Dragonfire.Saga emits structured logs, OpenTelemetry traces, and metrics automatically.
...
builder.Services.AddOpenTelemetry()
    .WithTracing(tracing => tracing
        .AddSource("Dragonfire.Saga")           // traces every workflow and step
        .AddOtlpExporter());
```
...
### Metrics (OpenTelemetry Meter `Dragonfire.Saga`)
...
All options live under the `"Dragonfire.Saga"` section in `appsettings.json`, or set programmatically.
...
├── src/
│   │
│   ├── Dragonfire.Saga.Core                          ← No infrastructure dependencies
...
│   │
│   ├── Dragonfire.Saga.Persistence.EfCore            ← SQL Server storage layer
...
│   │
│   └── Dragonfire.Saga.Extensions.DependencyInjection
...
│
├── tests/Dragonfire.Saga.Tests                       ← xUnit tests (in-memory, no SQL needed)
│
└── samples/Dragonfire.Saga.Sample                    ← Runnable example: OrderWorkflow saga
```
