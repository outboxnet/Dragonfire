# Dragonfire.Saga — Complete Technical Documentation

This document explains how Dragonfire.Saga works **behind the scenes**, from the moment you call `StartWorkflowAsync` to the moment a workflow completes or rolls back. It assumes you know basic C# but have never looked at a workflow engine before.
...
18. [Atomicity, idempotency, and what can go wrong](#18-atomicity-idempotency-and-what-can-go-wrong)
    ...
    - [What Dragonfire.Saga does NOT solve](#what-saganet-does-not-solve)
...
└─────────────────┬───────────────────────────────────────────────────────┘
                  ...
                  ▼
┌─────────────────────────────────────────────────────────────────────────┐
│  Dragonfire.Saga.Persistence.EfCore                                             │
│  ──────────────────────────                                             │
...
|---|---|---|
| **Engine** | `Dragonfire.Saga.Core` | All execution logic — no infrastructure dependencies |
| **Persistence** | `Dragonfire.Saga.Persistence.EfCore` | SQL Server storage via EF Core |
| **Wiring** | `Dragonfire.Saga.Extensions.DependencyInjection` | Registers everything in the ASP.NET DI container |

`Dragonfire.Saga.Core` only references `Microsoft.Extensions.*` abstractions (logging, hosting, DI interfaces). It has **zero** knowledge of EF Core or SQL Server. This means you could write a `Dragonfire.Saga.Persistence.Redis` or `Dragonfire.Saga.Persistence.Cosmos` implementation just by implementing `IWorkflowRepository`.
...
The engine uses `ActivitySource("Dragonfire.Saga")` to create spans:
...
All metrics use `Meter("Dragonfire.Saga")`:
...
This is the most important section for production use. It explains exactly what Dragonfire.Saga guarantees, what it does not guarantee, and what you must do yourself to build a correct system.
...
Dragonfire.Saga guarantees **at-least-once** execution of each step — meaning every step that runs will run to DB-confirmed completion eventually, but it may run more than once.
...
### What Dragonfire.Saga does NOT solve
...
| Compensation for steps with no undo | Manual intervention — Dragonfire.Saga marks the workflow `Failed` |
