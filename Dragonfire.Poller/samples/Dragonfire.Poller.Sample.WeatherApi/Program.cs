...
using Dragonfire.Poller.Sample.WeatherApi.Weather;
using Dragonfire.Poller.Services;

var builder = WebApplication.CreateBuilder(args);

// ── Dragonfire.Poller framework ──────────────────────────────────────────────────────────
...
builder.Services.AddSwaggerGen(c =>
{
    c.SwaggerDoc("v1", new()
    {
        Title       = "Dragonfire.Poller — Weather Sample API",
        Version     = "v1",
        Description =
            "Demonstrates the **Dragonfire.Poller** NuGet package against the free [Open-Meteo](https://open-meteo.com) " +
...
app.UseSwaggerUI(c =>
{
    c.SwaggerEndpoint("/swagger/v1/swagger.json", "Dragonfire.Poller Weather Sample v1");
    c.RoutePrefix = string.Empty;   // Serve Swagger UI at the root URL
    c.DocumentTitle = "Dragonfire.Poller Weather API";
