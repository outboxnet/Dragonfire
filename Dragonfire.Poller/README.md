...
[![NuGet](https://img.shields.io/nuget/v/Dragonfire.Poller.svg)](https://www.nuget.org/packages/Dragonfire.Poller)
[![CI](https://github.com/sandor/Dragonfire.Poller/actions/workflows/ci.yml/badge.svg)](https://github.com/sandor/Dragonfire.Poller/actions/workflows/ci.yml)
...
```bash
dotnet add package Dragonfire.Poller
```
...
`samples/Dragonfire.Poller.Sample.WeatherApi` demonstrates the full pattern against the free [Open-Meteo](https://open-meteo.com) weather API (no API key required).

```bash
cd samples/Dragonfire.Poller.Sample.WeatherApi
dotnet run
...
```bash
dotnet pack Dragonfire.Poller/Dragonfire.Poller.csproj -c Release -p:Version=1.0.0 -o ./nupkg
dotnet nuget push ./nupkg/Dragonfire.Poller.1.0.0.nupkg --api-key $NUGET_API_KEY --source https://api.nuget.org/v3/index.json
```
